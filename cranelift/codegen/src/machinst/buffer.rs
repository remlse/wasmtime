...
impl<I: VCodeInst> MachBuffer<I> {
    ...
    fn optimize_branches(&mut self) {
        ...
        if cfg!(fuzzing) {
            // I think 2 different optimize function with (#[cfg(fuzzing)] / #[cfg(not(fuzzing))]) would not work, because there would be no way to do bisection 
            ...
            // Generate an arbitrary boolean to skip optimizing or not... this value would later be generated by our control plane, to have more control for bisection
...
mod test {
 ...
 #[cfg(fuzzing)]
 pub fn test_fuzzing(){ // maybe pass in a seed?
